...
# --- Run FSL Baseline Experiments ---
echo ""
echo "**********************************************************************"
echo "Starting FSL Baseline Model Experiments"
echo "**********************************************************************"

# Script paths
PYTHON_EXECUTABLE="python3"
FSL_BASELINE_SCRIPT_PATH="src/baseline_fsl_evaluator.py"
PREPROCESS_SCRIPT_PATH="src/preprocess_only.py"

# Ensure results directory exists
mkdir -p "$RESULTS_DIR_MAIN_EXP"

# FSL Baseline configurations (matching LLM experiments)
FSL_BASELINE_MODELS_FEATURES=(
    "SVM|raw_hrrp"
    "SVM|scattering_centers"
    "RF|scattering_centers"
)

# Ensure data is preprocessed
if [ ! -d "data_processed/$DEFAULT_DATASET_KEY" ]; then
    echo "Preprocessed data not found. Running preprocessing..."
    if [ ! -f "$PREPROCESS_SCRIPT_PATH" ]; then
        cat > "$PREPROCESS_SCRIPT_PATH" << 'EOF'
#!/usr/bin/env python3
import sys
import os
...
from config import (
    AVAILABLE_DATASETS, TARGET_HRRP_LENGTH, PROCESSED_DATA_DIR,
    TEST_SPLIT_SIZE, RANDOM_STATE, SCATTERING_CENTER_EXTRACTION,
    SCATTERING_CENTER_ENCODING
)
from data_utils import prepare_npy_data_and_scattering_centers
...
class PreprocessConfig:
    def __init__(self):
        ...
        self.PREPROCESS_MAT_TO_NPY = True
        self.sc_extraction_config = SCATTERING_CENTER_EXTRACTION
        self.SCATTERING_CENTER_EXTRACTION = SCATTERING_CENTER_EXTRACTION
        self.sc_encoding_config = SCATTERING_CENTER_ENCODING

if __name__ == "__main__":
    print("=== Running data preprocessing ===")
    config = PreprocessConfig()
    prepare_npy_data_and_scattering_centers(config)
    print("=== Data preprocessing complete ===")
EOF
        chmod +x "$PREPROCESS_SCRIPT_PATH"
    fi
    "$PYTHON_EXECUTABLE" "$PREPROCESS_SCRIPT_PATH"
fi

# Use same FSL settings as LLM experiments
FSL_K_SHOT_VALUES=(1 5)  # Test with 1-shot and 5-shot

for k_shot in "${FSL_K_SHOT_VALUES[@]}"; do
    echo ""
    echo "===== Running FSL Baselines with K-shot = $k_shot ====="

    for baseline_config in "${FSL_BASELINE_MODELS_FEATURES[@]}"; do
        IFS='|' read -r model_type feature_type <<< "$baseline_config"

        CMD_ARGS_FSL_BASELINE=(
            "--dataset_key" "$DEFAULT_DATASET_KEY"
            "--model_type" "$model_type"
            "--feature_type" "$feature_type"
            "--n_way" "$DEFAULT_N_WAY"
            "--k_shot" "$k_shot"
            "--q_shot" "$DEFAULT_Q_QUERY"
            "--num_tasks" "$DEFAULT_NUM_FSL_TASKS"
            "--output_csv" "$BASELINE_RESULTS_CSV"
        )

        echo ""
        echo "--- FSL BASELINE: Model=$model_type, Features=$feature_type, K=$k_shot ---"
        echo "CMD: $PYTHON_EXECUTABLE $FSL_BASELINE_SCRIPT_PATH ${CMD_ARGS_FSL_BASELINE[*]}"

        "$PYTHON_EXECUTABLE" "$FSL_BASELINE_SCRIPT_PATH" "${CMD_ARGS_FSL_BASELINE[@]}"

        EXIT_CODE=$?
        if [ $EXIT_CODE -ne 0 ]; then
            echo "ERROR: FSL Baseline for $model_type ($feature_type, K=$k_shot) failed."
        else
            echo "Completed FSL Baseline: $model_type ($feature_type, K=$k_shot)"
        fi
...
echo ""
echo "**********************************************************************"
echo "All FSL Baseline Experiments Completed."
echo "Results saved to: $BASELINE_RESULTS_CSV"
echo "**********************************************************************"